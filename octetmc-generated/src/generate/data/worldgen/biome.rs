...
pub async fn biome(generated_dir : &Path, target_file : &Path) {
    ...
    while let Some(entry) = entries.try_next().await.unwrap() {
        ...
        let dry_foliage_colour = dry_foliage_colour.map_or(Cow::Borrowed("None"), |v| Cow::Owned(format!("Some(Rgb::from_u32({v}))")));
...
"    /// Vanilla `minecraft:{id}` damage type.
    pub const {ident} : Biome<'static> = Biome {{
        ...
        dry_foliage_colour    : {dry_foliage_colour},
...
struct Biome {
    ...
    #[expect(dead_code)]
    spawn_costs                : IgnoredAny,
    #[expect(dead_code)]
    spawners                   : IgnoredAny,
    temperature                : f32,
    #[expect(dead_code)]
    #[serde(default)]
    creature_spawn_probability : IgnoredAny,
    #[serde(default)]
